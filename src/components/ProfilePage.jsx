...
const ProfilePage = ({
  ...
  const renderCategoryIcon = (iconName) => {
    switch (iconName) {
      case "swirl":
        return <svg viewBox="0 0 24 24" fill="none" className="w-7 h-7 stroke-[#231C14]" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      default:
        return <span className="w-7 h-7 flex items-center justify-center">
  ...
  return <div className="w-full antialiased select-none bg-[#FAF8F5] min-h-screen font-baloo pb-12">
  ...
  />
    ...
    className={`flex items-center justify-center gap-2 py-2.5 rounded-lg border text-xs font-bold transition-all cursor-pointer ${theme === "light" ? "bg-[#FEF2CF] border-[#F2C200] text-amber-900 shadow-xs font-extrabold" : "bg-white border-[#EAE2D2] text-[#4A3E31] hover:bg-stone-50"}`}
  >
    ...
    className={`flex items-center justify-center gap-2 py-2.5 rounded-lg border text-xs font-bold transition-all cursor-pointer ${theme === "dark" ? "bg-[#121824] border-[#313B4D] text-white shadow-xs font-extrabold" : "bg-white border-[#EAE2D2] text-[#4A3E31] hover:bg-stone-50"}`}
  >
                      <Moon size={13} />
                      <span>Dark</span>
                    </button>
    ...
    className={`py-2.5 rounded-lg border text-xs font-bold transition-all cursor-pointer ${fontSize === "small" ? "bg-[#FEF2CF] border-[#F2C200] text-amber-900 shadow-xs font-extrabold" : "bg-white border-[#EAE2D2] text-[#4A3E31] hover:bg-stone-50"}`}
  >
                      A- Small
                    </button>
    ...
    className={`py-2.5 rounded-lg border text-xs font-bold transition-all cursor-pointer ${fontSize === "medium" ? "bg-[#FEF2CF] border-[#F2C200] text-amber-900 shadow-xs font-extrabold" : "bg-white border-[#EAE2D2] text-[#4A3E31] hover:bg-stone-50"}`}
  >
                      A Medium
                    </button>
    ...
    className={`py-2.5 rounded-lg border text-xs font-bold transition-all cursor-pointer ${fontSize === "large" ? "bg-[#FEF2CF] border-[#F2C200] text-amber-900 shadow-xs font-extrabold" : "bg-white border-[#EAE2D2] text-[#4A3E31] hover:bg-stone-50"}`}
  >
                      A+ Large
                    </button>
                  </div>
  ...
  >
    ...
    /* Yellow / White Pill list (Exactly matching colors) */
          ...
          <div className="flex flex-wrap gap-2 pt-1.5">
            {favorites.map((fav, index) => <button
    key={fav.label}
    onClick={() => toggleFavorite(index)}
    className={`flex items-center gap-1.5 text-[11px] font-black px-4 py-2 rounded-full tracking-wider uppercase border transition-all cursor-pointer ${fav.enabled ? "bg-[#FFCC00] border-[#E6B800] text-black font-black" : "bg-white border-[#EDE4D6] text-[#4A3E31] hover:bg-stone-50 font-extrabold"}`}
  >
                <span>{fav.label}</span>
                {fav.enabled ? <X size={12} className="stroke-[3.5] text-black/70" /> : <Plus size={12} className="stroke-[3.5] text-[#FF5F1F]" />}
              </button>)}
          </div>
        </div>
    ...
    className={`relative flex flex-col items-center justify-center p-4 rounded-xl border transition-all cursor-pointer ${cat.checked ? "bg-white border-[#D9D1C5] shadow-[0_1px_3px_rgba(0,0,0,0.01)]" : "bg-white border-[#EAE2D2] opacity-70 hover:opacity-100"}`}
    style={{ minHeight: "110px" }}
  >
                {
    /* Red-Orange checkbox in the top-right corner exactly like image */
  }
                <div className={`absolute top-2.5 right-2.5 w-4.5 h-4.5 rounded-md border flex items-center justify-center transition-all ${cat.checked ? "bg-[#FF5F1F] border-[#FF5F1F]" : "border-[#C4BBAF] bg-white"}`}>
