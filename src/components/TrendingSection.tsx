...
import { TRENDING_ITEMS, TrendingItem } from '../data/fandomData';

interface TrendingSectionProps {
  onSelectTrending: (item: TrendingItem) => void;
}

export const TrendingSection: React.FC<TrendingSectionProps> = ({ onSelectTrending }) => {
              ...
              <div>
                ...
                <div className={`relative ${idx % 2 === 0 ? 'aspect-[1/1]' : 'aspect-[4/3]'} w-full bg-[#0B0B0D] overflow-hidden`}>
                  ...
                  <button
                    ...
                    className={`absolute top-2.5 right-2.5 w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200 hover:scale-110 cursor-pointer ${
                      isSaved
                        ? 'bg-[#FF5F1F] text-white border border-[#FF5F1F]'
                        : 'bg-black/40 backdrop-blur-md border border-white/20 text-white hover:bg-black/70'
                    }`}
