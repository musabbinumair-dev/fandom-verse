...
export const ExplorePage: React.FC<ExplorePageProps> = ({ onOpenArticle }) => {
  ...
  const [savedIds, setSavedIds] = useState<Set<string>>(new Set(['c-1', 'c-4']));

  // All sample content explorer cards (Exactly matching the 10 cards in your image + extra for pagination)
  const [contentCards, setContentCards] = useState([
    ...
    {
      id: 'c-12',
      title: 'Joker (2019): Arthur Fleck Psychology Analysis',
      category: 'Movies',
      badgeColor: 'bg-rose-600',
      type: 'VIDEO',
      year: '2019',
      ...
      genre: 'Thriller',
      desc: 'A deep psychological profile of Arthur Fleck’s transformation into Gotham’s clown prince.'
    }
  ]);
  ...
  const filteredCards = contentCards.filter(card => {
    ...
    const matchesGenre = selectedGenre === 'All' || 
      card.genre.toLowerCase() === selectedGenre.toLowerCase();
    
    const matchesYear = selectedYear === 'All' || 
      card.year === selectedYear;

    const matchesType = selectedType === 'All' || 
      card.type === selectedType.toUpperCase();
  ...
  const renderCategoryIcon = (label: string) => {
    switch (label.toLowerCase()) {
      case 'anime':
        return (
          <svg viewBox="0 0 24 24" fill="none" className="w-[14px] h-[14px] stroke-current" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div className="w-full antialiased select-none bg-[#FAF8F5] min-h-screen font-baloo pb-16">
          ...
          {['All', 'Anime', 'Gaming', 'Movies', 'TV Shows', 'K-Pop', 'Comics', 'Manga', 'Cosplay'].map((cat) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 text-[11px] font-black px-4 py-2.5 rounded-full tracking-wider uppercase border transition-all cursor-pointer shrink-0 ${
                  isSelected 
                    ? 'bg-[#FFCC00] border-[#E6B800] text-black font-black shadow-xs' 
                    : 'bg-white border-[#EDE4D6] text-[#4A3E31] hover:bg-stone-50'
                }`}
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-4">
        {displayedCards.length === 0 ? (
          <div className="bg-white border border-[#EDE4D6] rounded-xl py-12 text-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            {displayedCards.map((card) => {
              ...
              // Helper to get 100% exact soft color styling for the category pill matching the image
              const getCategoryBadgeStyles = (catName: string) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative w-full h-[135px] overflow-hidden">
                    ...
                    >
                      {isSaved ? (
                        <BookmarkCheck size={13} className="text-[#FFCC00] stroke-[2.5]" />
                      ) : (
                        <Bookmark size={13} className="stroke-[2.5]" />
                      )}
                    </button>
