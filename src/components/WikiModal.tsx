...
import { X, BookOpen, Star, BookmarkPlus, Check } from 'lucide-react';
import { WikiItem } from '../data/fandomData';

interface WikiModalProps {
  wiki: WikiItem | null;
  onClose: () => void;
  onPinToRecent: (wikiId: string) => void;
  isPinned: boolean;
}
...
export const WikiModal: React.FC<WikiModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'popular' | 'guidelines'>('overview');
        ...
        <div className="relative h-44 sm:h-52 w-full bg-slate-900 overflow-hidden shrink-0">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg shadow-sm transition-all ${
                isPinned
