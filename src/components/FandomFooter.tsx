...
interface FandomFooterProps {
  onOpenLinkModal?: (title: string, content: string) => void;
}

export const FandomFooter: React.FC<FandomFooterProps> = ({ onOpenLinkModal }) => {
  const handleLinkClick = (e: React.MouseEvent, title: string, text: string) => {
    e.preventDefault();
    if (onOpenLinkModal) {
  ...
  return (
    <footer className="bg-[#0B0B0D] text-[#8A8A8E] pt-12 pb-10 px-4 sm:px-8 border-t border-[#232326] select-none">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-[11px] text-[#8A8A8E]">
          ...
          <div className="flex items-center gap-3">
            {['Discord', 'YouTube', 'Twitch', 'Reddit'].map((social) => (
              <button
                key={social}
