          ...
          {notifications.map((n) => (
            <div
              ...
              className={`p-3 rounded-lg border text-xs transition-colors ${
                n.unread
                  ? 'bg-[#021826] border-[#06F284]/40 text-[#F2F7F5]'
                  : 'bg-[#021826]/60 border-[#123847] text-[#7FA3A8]'
              }`}
            >
...
}> = ({ isOpen, onClose, onNavigateToDashboard }) => {
  if (!isOpen) return null;
...
}> = ({ isOpen, onClose, savedStories, onRemoveStory, onSelectStory }) => {
  if (!isOpen) return null;
      ...
      <div className="w-full max-w-sm bg-[#0A2A33] border-l border-[#123847] text-[#F2F7F5] h-full shadow-2xl p-5 flex flex-col animate-in slide-in-from-right duration-200">
        ...
        <div className="flex-1 overflow-y-auto py-3 space-y-3">
          {savedStories.length === 0 ? (
            <div className="text-center py-12 text-[#7FA3A8] text-xs">
...
export const UtilitiesModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  onRandomWiki: () => void;
  onOpenCreateDiscussion: () => void;
}> = ({ isOpen, onClose, onRandomWiki, onOpenCreateDiscussion }) => {
  if (!isOpen) return null;
