...
const ToolInteractiveModal = ({ tool, onClose }) => {
  ...
  const handleSimulateBattle = () => {
    ...
    setTimeout(() => {
      ...
      setBattleLog(
        `Battle Concluded! \u{1F3C6} Winner: ${winner} via Critical Concerto Resonance Burst with 4,820 remaining HP!`
      );
    }, 600);
  };
  ...
  >
    ...
    /* Interactive view based on tool type */
          ...
          {tool.id === "resonance-clash" && <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/80 space-y-3">
  ...
  >
              ...
              {battleLog && <div className="p-3 bg-pink-50 border border-pink-200 text-[#fa005a] rounded-lg text-xs font-semibold animate-in fade-in">
                  {battleLog}
                </div>}
            </div>}

          {tool.id === "tier-list-maker" && <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/80 space-y-3">
              <h4 className="text-xs font-bold uppercase text-slate-700">Quick Tier Board</h4>
              <div className="space-y-1.5 text-xs">
                {["S", "A", "B"].map((tier) => <div key={tier} className="flex items-center gap-2 p-1.5 bg-white rounded-lg border border-slate-200">
                    <span
    className={`w-7 h-7 rounded flex items-center justify-center font-black text-white shrink-0 ${tier === "S" ? "bg-red-500" : tier === "A" ? "bg-orange-500" : "bg-amber-500"}`}
  >
                      {tier}
                    </span>
                    <div className="flex-1 flex flex-wrap gap-1.5">
                      {tierItems.filter((item) => item.tier === tier).map((item, idx) => <span
    key={idx}
  ...
  >
          ...
          {tool.id === "warp-factor" && <div className="bg-slate-900 text-white rounded-xl p-4 space-y-3">
