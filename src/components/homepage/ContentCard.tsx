...
import { ContentItem } from '../../data/homepageData';
...
// Publisher Icons matching exact visual branding
const PublisherIcon: React.FC<{ type?: string; name?: string }> = ({ type }) => {
  if (type === 'rockstar') {
    return (
      <span className="w-3.5 h-3.5 rounded-[3px] bg-[#F7A600] text-[#111] font-black text-[8px] flex items-center justify-center font-sans shrink-0 leading-none select-none shadow-xs">
  ...
  if (type === 'ea') {
    return (
      <span className="w-3.5 h-3.5 rounded-full bg-white/25 text-white font-extrabold text-[7px] flex items-center justify-center font-sans shrink-0 leading-none select-none border border-white/40">
        EA
      </span>
    );
  }
  ...
  if (type === 'cdpr') {
    return (
      <span className="w-3.5 h-3.5 rounded-full bg-[#E52538] text-white font-black text-[7px] flex items-center justify-center shrink-0 leading-none select-none shadow-xs">
        ★
      </span>
    );
  }
  ...
  if (type === 'xbox') {
    return (
      <span className="w-3.5 h-3.5 rounded-full bg-[#107C10] text-white font-black text-[8px] flex items-center justify-center shrink-0 leading-none select-none">
        ✕
      </span>
    );
  }
  ...
  if (type === 'fromsoftware') {
    return (
      <span className="w-3.5 h-3.5 rounded-[3px] bg-white/35 text-white font-bold text-[6.5px] flex items-center justify-center shrink-0 leading-none select-none">
        FS
      </span>
    );
  }
  ...
  if (type === 'jump') {
    return (
      <span className="w-3.5 h-3.5 rounded-[3px] bg-[#E60012] text-white font-bold text-[6.5px] flex items-center justify-center shrink-0 leading-none select-none">
        JP
      </span>
    );
  }
  ...
  // Default brand icon
  return (
    <span className="w-3.5 h-3.5 rounded-full bg-white/25 text-white/90 font-bold text-[7px] flex items-center justify-center shrink-0 select-none border border-white/30">
      ✦
    </span>
  );
};
...
export const ContentCard: React.FC<ContentCardProps> = ({
  ...
  // Base tint for glassmorphism
  const baseTint = item.cardBgColor || '#2A302D';

  // Dynamic aspect ratio mapping for Pinterest-style bento card artwork
      ...
      <div className={`relative ${getArtworkAspect()} w-full overflow-hidden bg-black/40 rounded-none shrink-0`}>
        ...
        <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
          ...
          <button
            ...
            className={`w-7.5 h-7.5 rounded-none flex items-center justify-center transition-all duration-200 hover:scale-110 cursor-pointer shadow-lg ${
              localSaved
                ? 'bg-[#FF5F1F] text-white border border-[#FF5F1F]'
                : 'bg-black/55 hover:bg-black/80 backdrop-blur-md border border-white/25 text-white'
            }`}
