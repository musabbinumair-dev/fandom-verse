export interface UserSummary {
  ...
  memberSince: string;
  avatar: string;
  fandomsFollowedCount: number;
  savedItemsCount: number;
}

export interface FavoriteFandom {
  id: string;
  name: string;
  category: string;
...
export interface DashboardRecommendation {
  ...
  title: string;
  category: string;
  type: 'article' | 'character' | 'trailer' | 'event' | 'guide';
  actionLabel: string;
  image: string;
...
export interface ActivityItem {
  id: string;
  type: 'saved' | 'explored' | 'rated' | 'event' | 'character';
  title: string;
  meta: string;
  timeAgo: string;
  iconName: 'bookmark' | 'compass' | 'star' | 'calendar' | 'user';
  relatedId?: string;
  ratingValue?: string;
}
...
export const MOCK_FAVORITE_FANDOMS: FavoriteFandom[] = [
  {
    id: 'fandom-anime',
    name: 'Anime',
...
export const MOCK_CONTINUE_EXPLORING: DashboardRecommendation[] = [
  {
    id: 'rec-1',
...
export const MOCK_SAVED_ORBIT: SavedItem[] = [
  {
    id: 'save-1',
  ...
  {
    id: 'save-2',
    title: 'Pokémon Generation 10 Rumors and Competitive Meta Analysis',
    contentType: 'Discussion Thread',
    category: 'Pokémon Lore',
